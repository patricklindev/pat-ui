...
export const Checkbox:FC<ICheckboxProps> = (props)=> {
  ...
  const [markIcon,setMarkIcon] = useState(icon ? icon : 'check')
  ...
  return (
    <div className='checkbox-container'>
      <input 
        ...
        id={id} 
        checked={isCheck} 
        onChange={handleChange} // replace onChange CB if onChange prop exist
        disabled={disabled} 
      ...
      <label 
        ...
        <span 
          ...
          data-testid='iconBox'
          style={{
            border: markIcon !== 'check' ? 'none' : '', // remove border when icon is not check icon
          ...
          viewBox={IconPath[`${markIcon}`].viewBox} 
          path={IconPath[`${markIcon}`].path}
          fill={markIcon === 'check' ? fillCheck : fillIcon} // if icon is check icon implement color with fillCheck state else implement color with fillIcon state
