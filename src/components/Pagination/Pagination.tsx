...
const Pagination: React.FC<PaginationProps> = (props: PaginationProps) => {
    const { count, currentPage, onPrev, onNext, size, page, color, shape, disabled, siblingCount, itemTypes, updateCurrentPage }:
        {   updateCurrentPage: PageFunc,
            ...
            onChangePage?: (currentPage: number) => void; } = usePagination(props);
    ...
    const getSizeClass = (size: string|undefined) => {
        return size === 'lg' ? 'pagination__icons--lg' : size === 'sm' ? 'pagination__icons--sm' : 'pagination__icons--default';
    }
    const getShapeClass = (shape: string | undefined) => {
        return shape === 'round' ? 'pagination__icons--round' : shape === 'rounded' ? 'pagination__icons--rounded' : '';
    }
    const getColorClass = (color: string | undefined) => {
        return color === 'primary' ? 'pagination__icons--primary' : 'pagination__icons--secondary';
    }
  return (
    <div className={'pagination'}>
      {/* PAGINATION TO DO */}
      <div>
        <button onClick={onPrev} disabled={disabled || currentPage === 1}
                className={`pagination__icons__btn-icon pagination__icons__btn-arrow ${getDisabledClass(disabled)} ${getSizeClass(size)} ${getShapeClass(shape)}`}>
            ...
            { itemTypes.map((n, i) => {
                ...
                if (p === '') {
                    ...
                    return <button disabled={disabled} onClick={_=>updateCurrentPage(p)}
                                   className={`pagination__icons__btn-icon ${getDisabledClass(disabled)} ${getSizeClass(size)} ${getShapeClass(shape)}`}>
                        <div>{p}</div>
                        </button>
                    }
                })
      ...
      <div>
          <button onClick={onNext}
                  ...
                  className={`pagination__icons__btn-icon pagination__icons__btn-arrow
                     ${getDisabledClass(disabled)} ${getSizeClass(size)} ${getShapeClass(shape)}`}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
