...
describe('ValueLabel', () => {
    ...
    it("should display different color ValueLabel base upon color props", () => {
       //  primary color
       const ValueLabelP = render(<ValueLabel data-testid={'primary'} color='primary'/>).getByTestId('primary')
       expect(ValueLabelP).toBeInTheDocument()
       expect(ValueLabelP).toHaveClass('primary-color')

       // secondary color
        const ValueLabelS = render(<ValueLabel data-testid={'secondary'} color='secondary'/>).getByTestId('secondary')
